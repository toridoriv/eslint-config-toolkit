...
Generate a commit message that follows the gitmoji configuration under the `Configuration` section. Use the provided examples under the `Examples` section as a guide. The commit message should be concise, clear, and relevant to the changes made in the codebase. The project is a TypeScript library that export a set of utilities to configure ESLint, Prettier and EditorConfig out of the box. It's called `eslint-config-toolkit`.
...
- Explicitly copying the `stack` property to ensure it is preserved, handling cases where the stack property may be undefined.
```
